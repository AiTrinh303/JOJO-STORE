    ...
    <div className="flex mx-44 my-0">
      ...
      <div className="flex gap-4">
        <div className="flex flex-col gap-5">
          <img className="h-44 w-44 align-middle" src={product.image} alt="img1" />
          <img className="h-44 w-44 align-middle" src={product.image} alt="img2" />
          <img className="h-44 w-44 align-middle" src={product.image} alt="img3" />
          <img className="h-44 w-44 align-middle" src={product.image} alt="img4" />
        </div>
        <div className="img">
      ...
      <div className="ml-20 flex flex-col">
        ...
        <div className="mt-14">
          <h1 className="text-[#656565] text-xl font-semibold mb-5">Select Size</h1>
          <div className="flex my-7.5 gap-5 mb-5">
            {['S', 'M', 'L', 'XL', 'XXL'].map(size => (
              <div
                key={size}
                onClick={() => setSelectedSize(size)}
                className={`px-6 py-4 bg-[#fbfbfb] border border-[#ebebeb] rounded cursor-pointer ${selectedSize === size ? 'bg-darkgrey text-gray-800' : ''}`}
              >
                {size}
              </div>
            ))}
        ...
        <div className="flex items-center gap-2.5 mb-5">
          <button onClick={() => setQuantity(prev => (prev === 1 ? 1 : prev - 1))} className="w-[50px] h-[30px] flex items-center justify-center mb-5 cursor-pointer"> - </button>
