...
export default function SpinWheelGame() {
  ...
  const [code, setCode] = useState(null); // State để lưu trữ mã giảm giá

  const onFinished = (winner) => {
    console.log(winner);
    setResult(winner);
    // Tìm mã giảm giá tương ứng với kết quả winner
    const foundCode = saleCodes.find(item => {
      return item.sale === `JOJO${winner}`;
    });
    if (foundCode) {
      setCode(foundCode.sale);
    } else {
      setCode(null); // Nếu không tìm thấy mã, đặt code về null
    }
  };
