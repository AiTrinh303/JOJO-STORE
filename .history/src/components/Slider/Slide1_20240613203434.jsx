...
const Slide1 = ({ image }) => {
  ...
  function calculateCountdown() {
    ...
    targetDate.setDate(targetDate.getDate() + 30); // set 30 days from now
  ...
  return (
    <div className="w-full justifyflex-shrink-0 relative">
      ...
      <div className="absolute inset-0 flex flex-col justify-center items-center text-white bg-black bg-opacity-5">
        <div className="absolute top-24 text-start bg-transparent p-2.5">
          ...
          <p className="mt-32 text-red-800 text-5xl font-semibold countdown">{formatCountdown(countdown)}</p>
        </div>
      </div>
    </div>
  );
