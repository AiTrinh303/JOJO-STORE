...
const Slide1 = ({ image }) => {
  ...
  return (
    <div className="w-full flex-shrink-0 relative">
      <img src={image} alt="" className="object-contain w-full h-auto max-h-full" />
      <div className="absolute inset-0 flex flex-col justify-center items-center text-white bg-black bg-opacity-5">
        <div className="absolute top-24 text-start bg-transparent p-2.5">
          ...
          <p className="mt-32 text-red-600 text-5xl font-semibold countdown">{formatCountdown(countdown)}</p>
        </div>
      </div>
    </div>
  );
