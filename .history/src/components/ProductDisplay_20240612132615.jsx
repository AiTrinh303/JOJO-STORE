    ...
    <div className="flex mx-44 my-0">
      ...
      <div className="ml-20 flex flex-col">
        ...
        <div className="mt-14">
          ...
          <div className="flex my-7.5 gap-5 mb-5 mt-5">
            {['S', 'M', 'L', 'XL', 'XXL'].map(size => (
              <div
                key={size}
                onClick={() => setSelectedSize(size)}
                className={`px-6 py-4 bg-gray-100 border border-[#ebebeb] rounded cursor-pointer hover:bg-gray-400`}
              >
                {size}
              </div>
            ))}
        ...
        <div className="flex items-center gap-2.5 mb-5 mt-5">
          <button onClick={() => setQuantity(prev => (prev === 1 ? 1 : prev - 1))} className="w-11 h-7 flex items-center justify-center mb-5 cursor-pointer bg-gray-200 hover:bg-gray-400"> - </button>
          <span className="w-8 h-8 flex items-center justify-center mb-5">{quantity}</span>
          <button onClick={() => setQuantity(prev => prev + 1)} className="w-11 h-7 flex items-center justify-center border-none mb-5 cursor-pointer bg-gray-200 hover:bg-gray-400"> + </button>
        </div>
