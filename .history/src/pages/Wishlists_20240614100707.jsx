...
const Wishlists = () => {
  ...
  return (
    <div className="mx-16 my-10 px-5">
      ...
      {wishlist.length === 0 ? (
        ...
        wishlist.map((product) => (
          <div className="flex justify-start items-center py-8 px-5" key={product.id}>
            <img className="h-16 w-16 object-cover mr-56" src={product.image} alt={product.title} />
            <p className="w-2/6 mr-3">{product.title}</p>
            <p className="w-1/6 mr-1">${product.new_price.toFixed(2)}</p>
            <button 
